...
public class BookApiController { // 컴포지션 = has 관계
    ...
    // 3. 책한건보기
    @GetMapping("/api/vi/book/{id}")
    public ResponseEntity<?> getBookOne(@PathVariable Long id) {
        BookRespDto bookRespDto = bookService.책한건보기(id);
        return new ResponseEntity<>(CMSRespDto.builder().code(1).msg("글 한건보기 성공").body(bookRespDto).build(),
                HttpStatus.OK); // 200 = ok
    }
    ...
    public ResponseEntity<?> deleteBook(@PathVariable Long id) {
        ...
        return new ResponseEntity<>(CMSRespDto.builder().code(1).msg("글 삭제하기 성공").body(null).build(),
                HttpStatus.OK); // 200 = ok
    }
    ...
    public ResponseEntity<?> updateBook(@PathVariable Long id, @RequestBody @Valid BookSaveReqDto bookSaveReqDto, BindingResult bindingResult) {
        ...
        return new ResponseEntity<>(CMSRespDto.builder().code(1).msg("글 수정하기 성공").body(bookRespDto).build(),
                HttpStatus.OK); // 200 = ok
    }
}